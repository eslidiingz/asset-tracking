...
export const usePort = () => {
    ...
    const fetchPorts = async () => {
        ...
        try {
            // ระบุ generic type ให้ $fetch เพื่อลดความซับซ้อนของ type inference
            const response = await $fetch<ApiResponse>('/api/ports')
    ...
    const findPort = async (id: number | string) => {
        isLoading.value = true;
        try {
            const response = await $fetch<ApiResponse>(`/api/ports/${id}`)
    ...
    const updatePort = async (updatedPort: Port) => {
        isLoading.value = true;
        try {
            const response = await $fetch<ApiResponse>(`/api/ports/${updatedPort.id}`, {
                method: 'PUT',
                body: updatedPort
            })
    ...
    const deletePort = async (portToDelete: Port) => {
        isLoading.value = true;
        try {
            const response = await $fetch<ApiResponse>(`/api/ports/${portToDelete.id}`, {
                method: 'DELETE'
            })
