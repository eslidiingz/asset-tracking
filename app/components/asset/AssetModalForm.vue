...
<template>
    <Dialog v-model:visible="visible" modal :header="asset ? 'Edit Asset' : 'Add Asset'"
        ...
        <Form @submit="onSubmit" class="mt-2">
            <div class="space-y-2 mb-6">
                <div class="flex flex-col gap-1.5">
                    <InputText id="name" name="name" type="text" placeholder="e.g. Crypto, US Stocks, Gold" required
                        v-model="form.name" class="w-full h-12" autofocus />
                </div>
            ...
            <div class="flex flex-col sm:flex-row justify-end gap-3 mt-4">
                ...
                <Button type="submit" :loading="isLoading" class="order-1 sm:order-2 h-12 px-8">
                    ...
                    <span>{{ asset ? 'Update Changes' : 'Create Asset' }}</span>
                </Button>
            </div>
        </Form>
    </Dialog>
